...
export const todoApp = ({ todos, stats, error }: TodoAppProps) => html`
  <section id="todo-app" class="grid gap-6 xl:grid-cols-[23rem_1fr]">
    ...
    <section class="card border border-base-content/10 bg-base-100/95 shadow-xl">
      <div class="card-body gap-0 p-0">
        <div class="flex flex-col gap-3 border-b border-base-content/10 px-5 py-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p class="font-serif text-sm italic text-accent">Quadro em memória</p>
            <h2 class="text-2xl font-black tracking-normal">Tarefas em curso</h2>
          </div>
          <span class="badge badge-neutral badge-lg">${pluralize(stats.active, 'ativa', 'ativas')}</span>
        </div>
...
export const todoForm = (error?: string) => html`
  <form
    class="card border border-primary/20 bg-primary text-primary-content shadow-xl"
    hx-post="/todos"
    hx-target="#todo-app"
    hx-swap="outerHTML"
  >
    <div class="card-body gap-4 p-5">
      <div>
        <p class="font-serif text-sm italic opacity-80">Entrada rápida</p>
      ...
      <label class="form-control gap-2">
        <span class="label p-0">
          <span class="label-text font-bold text-primary-content">Título</span>
        </span>
        <input
          name="title"
          type="text"
          ...
          class="input input-bordered bg-base-100 text-base-content placeholder:text-base-content/45"
        />
      </label>

      <label class="form-control gap-2">
        <span class="label p-0">
          <span class="label-text font-bold text-primary-content">Notas</span>
        </span>
        <textarea
          name="notes"
          rows="4"
          ...
          class="textarea textarea-bordered resize-none bg-base-100 text-base-content placeholder:text-base-content/45"
        ></textarea>
      </label>

      <button type="submit" class="btn btn-accent btn-block">Criar tarefa</button>
    </div>
  </form>
`

export const todoStats = (stats: TodoStats) => html`
  <div class="stats stats-vertical w-full border border-base-content/10 bg-base-100/95 shadow-xl">
    <div class="stat">
...
export const emptyState = () => html`
  <div class="grid min-h-80 place-items-center px-5 py-12">
    <div class="max-w-md text-center">
      <div class="mx-auto mb-5 grid size-16 place-items-center rounded-full border border-dashed border-base-content/30 bg-base-200">
        <span class="font-serif text-3xl italic">0</span>
      </div>
      <h3 class="text-2xl font-black tracking-normal">Nenhuma tarefa no quadro</h3>
      <p class="mt-3 text-base-content/65">
        Crie a primeira tarefa para movimentar a mesa. Os dados ficam apenas no SQLite em memória.
      </p>
    </div>
  </div>
`
...
export const todoItem = (todo: Todo) => {
  ...
  return html`
    <li id="todo-${todo.id}" class="group grid gap-4 px-5 py-5 md:grid-cols-[auto_1fr_auto] md:items-start">
      <input
